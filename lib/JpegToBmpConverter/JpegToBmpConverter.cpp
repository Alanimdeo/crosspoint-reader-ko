...
// quantize1bit and Atkinson1BitDitherer are defined in BitmapHelpers.h
  ...
  uint16_t acScale_fp = 256;  // 1.0 in 8.8 fixed point (256 = no scaling)
  {
    ...
    if (ok) {
      ...
      if (range > 0 && range < 200) {
        ...
        LOG_DBG("JPG", "Auto-contrast: global min=%d max=%d range=%d, stretching", globalMin, globalMax, range);
      } else {
    ...
    for (int y = startRow; y < endRow && y < imageInfo.m_height; y++) {
      ...
      if (!needsScaling) {
        ...
        if (written != static_cast<size_t>(bytesPerRow)) {
          LOG_ERR("JPG", "Write error at row %d: wrote %d of %d bytes", totalRowsWritten, written, bytesPerRow);
          writeError = true;
        }
        totalRowsWritten++;
      } else {
