...
// it. The dictionary entry is rebuilt per call from the folders discovered under
// /dictionaries.
//
// The Korean build hides the upstream font-family/font-size pickers (reader font
// selection routes through the FontSelection action) and gates the Focus Reading
// toggle to English, so the per-call copy is also where the language filter runs.
inline std::vector<SettingInfo> getSettingsList(const std::vector<DictionaryEntry>* dictionaries = nullptr) {
  static const std::vector<SettingInfo> baseList = [] {
    ...
    std::vector<SettingInfo> v = {
        ...
        // SD .epdfont (FontSelectionActivity, Target::Reader). Upstream's built-in
        // NotoSerif/NotoSans families are not loaded in this build, so neither picker would
        // select anything renderable.
