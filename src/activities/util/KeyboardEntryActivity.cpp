...
fui::KeyboardLayoutId layoutForLanguage(const Language language) {
  // The Korean build ships only english.yaml and korean.yaml, so the generated Language enum has
  // no FR/DE/ES members to switch on and every language uses the QWERTY layout. Upstream's
  // AZERTY/QWERTZ/Spanish cases return here if those translations are ever added back.
  (void)language;
