...
void OtaUpdateActivity::render(RenderLock&&) {
  ...
  if (state == CHECKING_FOR_UPDATE) {
    ...
    // Upstream's translated reason (wrong-device) wins when it is set; otherwise fall back to the
    // Korean updater's raw failure tag, which distinguishes http/flash/SD failures during support.
